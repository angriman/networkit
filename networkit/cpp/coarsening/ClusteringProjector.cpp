...
Partition ClusteringProjector::projectBack(const Graph &, const Graph &Gfine,
...
Partition ClusteringProjector::projectBackToFinest(const Partition &zetaCoarse,
    ...
    Gfinest.parallelForNodes([&](node v) { tempMap[v] = v; });

    // find coarsest supernode for each node
    for (const auto &map : maps)
        Gfinest.parallelForNodes([&](node v) { tempMap[v] = map[tempMap[v]]; });
...
Partition ClusteringProjector::projectCoarseGraphToFinestClustering(
    const Graph &Gcoarse, const Graph &Gfinest, const std::vector<std::vector<node>> &maps) {
    ...
    // initialize to identity
    Gfinest.parallelForNodes([&](node v) { super[v] = v; });

    // find coarsest supernode for each node
    for (const auto &map : maps)
        Gfinest.parallelForNodes([&](node v) { super[v] = map[super[v]]; });

    // assign super node id as cluster id
    Gfinest.parallelForNodes([&](node v) { zeta[v] = super[v]; });
