...
DynKatzCentrality::DynKatzCentrality(const Graph &G, count k, bool groupOnly, double tolerance)
    ...
    if (maxdeg == 0)
        throw std::runtime_error(
            "Alpha is chosen based on the max. degree; therefore, that degree must not be zero");
...
bool pairCompare(const std::pair<node, double> &firstElem,
                 const std::pair<node, double> &secondElem) {
    return firstElem.second > secondElem.second;
}

void DynKatzCentrality::run() {
    DEBUG("DynKatz: Nodes: ", G.numberOfNodes(), ", edges: ", G.numberOfEdges(), ", graph is ",
...
#pragma omp parallel for
            ...
            G.balancedParallelForNodes([&](node u) {
                nPaths[i][u] = 0;
                G.forInEdgesOf(u, [&](node v, edgeweight) { nPaths[i][u] += preUpdateContrib[v]; });
    ...
    if (activeRanking.size() >= k)
        reactivation_threshold =
            baseData[*std::min_element(activeRanking.begin(), activeRanking.end(),
                                       [&](node u, node v) { return baseData[u] < baseData[v]; })];
...
void DynKatzCentrality::doIteration() {
    ...
    auto bound_factor = next_alpha_pow * (1 / (1 - alpha * maxdeg));
    G.balancedParallelForNodes([&](node u) {
        G.forInEdgesOf(u, [&](node v, edgeweight) { nPaths[r][u] += nPaths[r - 1][v]; });
...
bool DynKatzCentrality::checkConvergence() {
    if (useQueue) {
        if (!filledQueue) {
            G.forNodes([&](node u) { activeQueue.insert(scoreData[u], u); });
        ...
        if (activeRanking.size() > k) {
            ...
            sort_timer.start();
            std::partial_sort(activeRanking.begin(), activeRanking.begin() + k, activeRanking.end(),
                              [&](node u, node v) { return scoreData[u] > scoreData[v]; });
            sort_timer.stop();
            ...
            activeRanking.erase(std::remove_if(activeRanking.begin() + k, activeRanking.end(),
                                               [&](node u) { return !isActive[u]; }),
                                activeRanking.end());
        }
