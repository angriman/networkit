...
public:
    ...
    TemporaryFile(size_t bytes, std::default_random_engine &prng)
        : dataBegin(new value_type[bytes]), dataEnd(dataBegin.get() + bytes) {
        // fill array with random data
        {
            ...
            std::generate(dataBegin.get(), dataEnd,
                          [&distr, &prng] { return static_cast<value_type>(distr(prng)); });
        }

        // obtain temporary file name (C++17 offers std::filesystem, which we currently cannot rely
        // on)
#ifdef NETWORKIT_WINDOWS
        {
            char cpath[MAX_PATH];
...
INSTANTIATE_TEST_SUITE_P(MemoryMappedFileIOGTest, MemoryMappedFileIOGTest,
                         ::testing::Values(0, 1, (1 << 2) - 1, (1 << 2), (1 << 2) + 1,
                                           (1 << 10) - 1, (1 << 10), (1 << 10) + 1, (1 << 16) - 1,
                                           (1 << 16), (1 << 16) + 1, (1 << 22) - 1, (1 << 22),
                                           (1 << 22) + 1));
