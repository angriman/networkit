...
#pragma omp for nowait
        for (omp_index i = 0; i < static_cast<omp_index>(nodes.size()); ++i) {
            ...
            // Discard all node-pairs of the form u > v. This removes all duplicates that result
            // from undirected edges.
            missingAtU.erase(
                std::remove_if(std::begin(missingAtU), std::end(missingAtU),
                               [&](std::pair<node, node> p) { return p.first >= p.second; }),
                std::end(missingAtU));
            missingLinksPrivate.insert(missingLinksPrivate.end(), missingAtU.begin(),
                                       missingAtU.end());
        }
#pragma omp critical
        missingLinks.insert(missingLinks.end(), missingLinksPrivate.begin(),
                            missingLinksPrivate.end());
    }
