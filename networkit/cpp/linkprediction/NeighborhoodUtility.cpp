...
NeighborhoodUtility::getSortedNeighborhoods(const Graph &G, node u, node v) {
...
std::vector<node> NeighborhoodUtility::getNeighborsUnion(const Graph &G, node u, node v) {
    ...
    std::set_union(neighborhoods.first.begin(), neighborhoods.first.end(),
                   neighborhoods.second.begin(), neighborhoods.second.end(),
...
std::vector<node> NeighborhoodUtility::getCommonNeighbors(const Graph &G, node u, node v) {
    ...
    std::set_intersection(neighborhoods.first.begin(), neighborhoods.first.end(),
                          neighborhoods.second.begin(), neighborhoods.second.end(),
                          std::back_inserter(commonNeighbors));
