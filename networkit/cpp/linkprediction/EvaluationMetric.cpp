...
EvaluationMetric::getCurve(std::vector<LinkPredictor::prediction> predictions,
                           count numThresholds) {
    ...
    // Don't overshoot with the number of thresholds being greater than the number of predictions
    // + 1.
    ...
    for (index i = 0; i < numThresholds; ++i) {
        ...
        thresholdSet.insert(
            std::ceil(numPredictions * (1.0 * i / static_cast<double>(numThresholds - 1))));
    }
...
double EvaluationMetric::getAreaUnderCurve(
    std::pair<std::vector<double>, std::vector<double>> curve) const {
    ...
    for (index i = 0; i < curve.first.size() - 1; ++i) {
        // Trapezoid rule
        AUC +=
            0.5 * (curve.first[i + 1] - curve.first[i]) * (curve.second[i] + curve.second[i + 1]);
    }
    return AUC;
}
...
void EvaluationMetric::setTrueAndFalseNegatives() {
    ...
    for (index i = predictions.size(); i > 0; --i) {
        ...
        bool hasEdge =
            testGraph->hasEdge(predictions[i - 1].first.first, predictions[i - 1].first.second);
        if (hasEdge) {
            tmpFalseNegatives++;
        } else {
            tmpTrueNegatives++;
...
void EvaluationMetric::sortPointsOfCurve(
    std::pair<std::vector<double>, std::vector<double>> &curve) const {
    ...
    Aux::Parallel::sort(permutation.begin(), permutation.end(),
                        [&](int i, int j) { return curve.first[i] < curve.first[j]; });
    ...
    std::transform(permutation.begin(), permutation.end(), sortedY.begin(),
                   [&](int i) { return curve.second[i]; });
