...
std::pair<std::vector<double>, std::vector<double>> PrecisionRecallMetric::generatePoints() {
    ...
    for (index i = 0; i < thresholds.size(); ++i) {
        ...
        if (truePositives.at(i) > 0 || falseNegatives.at(i) > 0) {
            recall = 1.0 * static_cast<double>(truePositives.at(i))
                     / static_cast<double>(truePositives.at(i) + falseNegatives.at(i));
        }
        if (truePositives.at(i) > 0 || falsePositives.at(i) > 0) {
            precision = 1.0 * static_cast<double>(truePositives.at(i))
                        / static_cast<double>(truePositives.at(i) + falsePositives.at(i));
        }
