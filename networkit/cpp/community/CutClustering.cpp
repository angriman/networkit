...
NetworKit::CutClustering::CutClustering(const Graph &G, NetworKit::edgeweight alpha)
    : CommunityDetectionAlgorithm(G), alpha(alpha) {}
...
void NetworKit::CutClustering::run() {
    ...
    for (node u : sortedNodes) {
        ...
        if (!result.contains(u)) {
            ...
            // all nodes in the source side form a new cluster, this cluster might absorb other
            // clusters
...
std::map<NetworKit::edgeweight, NetworKit::Partition>
NetworKit::CutClustering::getClusterHierarchy(const NetworKit::Graph &G) {
    ...
    if (G.isWeighted()) {
        ...
        G.forEdges([&](node, node, edgeweight weight) { upper = std::max(weight, upper); });
        upper += 1;
    }
    ...
    G.parallelForNodes([&](node u) {
        // we do not care which representative is actually used so we simply take the one that wins
        // this obvious race condition
    ...
    G.parallelForNodes(
        [&](node u) { lowerClusters[u] = componentReps[connComp.componentOfNode(u)]; });
    ...
    if (connComp.numberOfComponents() > 1) {
        ...
        result.insert(
            std::make_pair(-1, std::move(wholeGraph))); // wholeGraph won't be used anymore
    }

    // Construct the upper bound for the calculation. It will be inserted into the hierarchy later
    // when the lower bound for the parameter range in which this clustering is returned has been
    // determined.
...
void NetworKit::CutClustering::clusterHierarchyRecursion(
    const NetworKit::Graph &G, edgeweight lower, NetworKit::Partition lowerClusters,
    edgeweight upper, NetworKit::Partition upperClusters, std::map<edgeweight, Partition> &result) {
    while (true) {
        edgeweight middle = -1;

        // calculate cut values and cluster sizes for all clusters
        // FIXME: it might be more efficient to calculate them only on demand, especially the cut
        // values. However without a possibility to efficiently enumerate the nodes of a cluster
        // this is inefficient.
        ...
        for (auto it : lowerSizes) {
            ...
            // find the highest possible breakpoint, first candidate: the upper cluster in which the
            // representative of the lower cluster is.
            ...
            // check if we have actually found a new breakpoint. Use an epsilon in order to be sure
            // that the (almost) same value is not used twice.
            if (possibleBreakpoint + std::numeric_limits<edgeweight>::epsilon() < upper) {
                // also the actual alpha value needs to be higher as otherwise the lower clustering
                // might be calculated again because the value is not high enough.
                middle = possibleBreakpoint + std::numeric_limits<edgeweight>::epsilon();
                break;
            } else if (possibleBreakpoint > upper) {
                // this shouldn't happen and indicates that there are numerical inaccuracies or an
                // error in the implementation
                WARN("Warning: possible breakpoint ", possibleBreakpoint,
                     " higher than the upper value: ", upper);
            }
        }
