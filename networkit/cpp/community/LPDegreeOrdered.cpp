...
void LPDegreeOrdered::run() {
    ...
    auto propagateLabels = [&](node v) {
        if ((active[v]) && (G->degree(v) > 0)) {
            ...
            // get dominant label
            label dominant = std::max_element(labelCounts.begin(), labelCounts.end(),
                                              [](const std::pair<label, count> &p1,
                                                 const std::pair<label, count> &p2) {
            ...
            if (labels[v] != dominant) {          // UPDATE
                ...
                G->forNeighborsOf(v, [&](node u) { active[u] = 1; });
            } else {
                active[v] = 0;
            }
    ...
    G->forNodes([&](node v) { nodes.push_back(v); });
