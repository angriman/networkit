...
public:
    ...
    std::vector<double> computeDiagonal(double epsilon, double tol);
    ...
    std::vector<double> computeDiagonalRandomEst(count k, double tol);

    /**
     * Compute approximation with random vectors estimator (Hutchinson estimator). This solves k
     * linear systems in parallel, where k is the number of samples, (should be log(n)/(tol^2)).
     */

    std::vector<double> computeDiagonalHadaEst(count k, double tol);

    /**
     * Compute approximation with Hadamard vectors. This solves k linear systems in parallel, where
     * k is the number of samples, (should be log(n)/(tol^2)).
     */
